...
В настоящем разделе чётко разграничиваются результаты, заимствованные
из диссертации von Hippel (2025), и результаты, полученные автором
самостоятельно.

**Заимствовано из von Hippel (2025):**

- Определения Процесса (Definition 2, p. 63) и параллельной композиции
  (Definition 3, p. 64) как методологическая основа построения модели.
- Формальные описания компонентов: `senderR` (формула 3.1, p. 39),
  `receiverR` (формула 3.2, p. 40), `tbfR` (формулы 3.3–3.6, p. 42–43),
  `sysR` (формула 3.11, p. 46).
- Инварианты Theorem 1 (p. 39–40) и Theorem 2 (p. 40–41) как
  верифицируемые свойства.
- Теорема о композиционности TBF (Theorem 4, p. 44–46).
- Конечная абстракция N=2 как стандартная практика model checking.

**Вклад автора:**

- **JSON-описание модели K_GBN** — формализация компонентов и переходов
  в машиночитаемом формате, пригодном для автоматической генерации SMV.
- **CTL-спецификации** (раздел 3.X.5) — переформулировка инвариантов
  Theorem 1–2 и свойств эффективности в язык CTL для верификации в nuXmv.
  Спецификации не заимствованы из диссертации: von Hippel использует
  ACL2s и Ivy, а не CTL/nuXmv.
- **Итерационное уточнение φ₃** (раздел 3.X.10): обнаружение контрпримера,
  его анализ и уточнение формулы `AG (window_full → F transmitting)` до
  `AG ((window_full ∧ ¬Connection_complete) → F transmitting)`.
- **Верификация моделей** инструментом nuXmv 2.1.0 и получение результатов.
- **Интерпретация результатов** в контексте планирования канальных ресурсов.
...
**Отношение переходов `senderR`** (von Hippel (2025), p. 39, формула 3.1)**:**
...
**Инварианты (Теорема 1, von Hippel (2025), p. 39-40):**
...
**Инвариант (Теорема 2, von Hippel (2025), p. 40-41):**
...
(Definition 3, von Hippel (2025), p. 64-65):
...
$$
*«ACK никогда не опережает отправленные пакеты»* — Inv1 из Теоремы 1 (p. 39).
