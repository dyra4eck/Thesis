...
**Заимствовано из von Hippel (2025) и RFC 6298:**

- Алгоритм Карна (Algorithm 1, p. 19) и условие ok-to-sample (p. 20)
  как описание верифицируемого механизма.
- Формулы вычисления rto, srtt, rttvar по RFC 6298 (p. 21) как
  содержательная основа состояний модели.
- Наблюдения 5 и 6 (p. 22–23) о сходимости srtt и rttvar как
  теоретическое обоснование верифицируемых свойств.
- Описание патологического случая бесконечных таймаутов (p. 23) как
  обоснование состояния `timeout_risk` и свойства φ₅.

**Вклад автора:**

- **Качественная абстракция** числовых переменных (srtt, rttvar, rto)
  в конечное множество фаз — оригинальное решение, позволяющее применить
  model checking к механизму с бесконечным пространством состояний.
- **JSON-описание модели K_RTO** (15 состояний, 22 перехода).
- **CTL-спецификации** (раздел 3.Y.6) — переформулировка Наблюдений 5–6
  в язык CTL. Von Hippel доказывает эти свойства в ACL2s (ε/δ-доказательства
  для рациональных чисел); настоящая работа верифицирует их качественные
  аналоги методом model checking.
- **Введение состояния `s_abort`** как формализация механизма
  `tcp_retries_max` — оригинальное расширение модели, потребовавшееся
  по результатам анализа контрпримера.
- **Три итерации уточнения** (раздел 3.Y.7): анализ контрпримеров,
  уточнение φ₃ и введение ограничения `FAIRNESS`.
