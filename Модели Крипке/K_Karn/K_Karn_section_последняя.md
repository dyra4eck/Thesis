...
**Заимствовано из von Hippel (2025):**

- Algorithm 1 (p. 19) — полное описание алгоритма Карна как
  верифицируемого механизма.
- Наблюдения 1–4 (p. 17–21) как теоретическая основа верифицируемых свойств:
  монотонность ACK (Obs.1), инварианты I1/I2 (Obs.2), пессимистичность
  выборки (Obs.3), точность в FIFO-канале (Obs.4).
- Понятие немешающего монитора (non-interfering monitor, p. 17) как
  принцип архитектуры модели.

**Вклад автора:**

- **Интерпретация алгоритма Карна как Процесса** (Definition 2, p. 63)
  с явными входами I = {snds, rcvs} и выходами O = {S} — оригинальный
  шаг, обеспечивающий встраивание монитора в рамки methodology данной работы.
- **JSON-описание модели K_Karn** (12 состояний, 19 переходов) с
  привязкой каждого состояния к конкретным строкам Algorithm 1.
- **CTL-спецификации** (раздел 3.Z.6) — прямая CTL-верификация
  Наблюдений 1–4. Von Hippel доказывает эти наблюдения в Ivy (инвариантная
  индукция); настоящая работа проверяет их CTL-аналоги в nuXmv.
- **Две итерации уточнения** (раздел 3.Z.7):
  — Исправление семантики предиката `ACK_fresh` в состояниях s8/s9.
  — Расширение φ₈ за счёт выявления `ACK_stale` как законного исхода
    неоднозначного окна (строка 12 Algorithm 1, p. 19).
- **Ограничение справедливости** `FAIRNESS Sample_produced | Karn_done |
  ACK_stale` — оригинальная формализация семантического допущения о
  конечности числа ретрансмиссий.
- **Верификация** инструментом nuXmv 2.1.0 и получение результатов.
